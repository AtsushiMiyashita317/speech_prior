...
class Embedding(NetworkModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.bias is not None:
            dx = dx + self.b * math.sqrt(abs(self.beta))
            x0 = x0 + self.b0 * math.sqrt(abs(self.beta))
            x = x + self.bias * math.sqrt(abs(self.beta))
...
class Linear(NetworkModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x * math.sqrt(abs(self.alpha))
        dx = dx * math.sqrt(abs(self.alpha))
        x0 = x0 * math.sqrt(abs(self.alpha))
...
class Conv1d1x1(NetworkModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        weight = self.weight * math.sqrt(abs(self.alpha))
        w0 = self.w0 * math.sqrt(abs(self.alpha))
        w = self.w * math.sqrt(abs(self.alpha))
...
class Conv1d(NetworkModule):
    def __init__(
        ...
        if alpha is None:
            self.register_buffer("alpha", torch.tensor(2.0) / math.sqrt(kernel_size))
